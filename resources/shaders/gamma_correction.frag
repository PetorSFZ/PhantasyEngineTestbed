...
// Currently selected gamma curve can be visualized with this graph:
// https://www.desmos.com/calculator/qqga8crnaa

const float A = 0.25; // Shoulder strength (default: 0.22)
const float B = 0.25; // Linear strength (default: 0.30)
const float C = 0.105; // Linear angle (default: 0.10)
const float D = 0.20; // Toe strength (default: 0.20)
const float E = 0.025; // Toe numerator (default: 0.01)
const float F = 0.25; // Toe denominator (default: 0.30)
const float W = 11.2; // Linear white point value (default: 11.2)
const float BLACK_CRUSH = 0.0015; // Controls how much blacks should be crushed
...
vec4 applyU2Tonemap(vec4 linearValue)
{
	...
	vec3 x = exposureBias * max(vec3(0.0), linearValue.rgb - BLACK_CRUSH);
	vec3 res = u2Tonemap(x) / u2Tonemap(vec3(W));

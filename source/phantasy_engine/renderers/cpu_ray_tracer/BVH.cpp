...
void BVH::buildStaticFrom(const StaticScene& scene) noexcept
{
	...
	for (const DynArray<Renderable>* renderableList : {&scene.opaqueRenderables, &scene.transparentRenderables}) {
		for (const Renderable& renderable : *renderableList) {
	...
	buildStaticFrom(inTriangles);
}

void BVH::buildStaticFrom(const DynArray<TriangleVertices>& inTriangles) noexcept
{
	...
	nodes.add(BVHNode());
	fillStaticNode(0, 0, triangleInds, inTriangles, inTriangleAabbs);
}
